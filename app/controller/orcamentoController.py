...
class orcamentoController:
    ...
    def consultarOrcamentos(self, idEmpreend):
        ...
        query = "select mes_vigencia, ano_vigencia, dt_carga from " + MySql.DB_NAME + \
            ".tb_orcamentos where id_empreendimento = " + \
...
#            m.setIdEmpreend(x[1])
    ...
    def consultarOrcamentoPelaVigencia(self, idEmpreend, mes, ano):
        query = f"SELECT id_orcamento, id_empreendimento, mes_vigencia, ano_vigencia, dt_carga, item, orcado_valor, fisico_valor, fisico_percentual, fisico_saldo, financeiro_valor, financeiro_percentual, financeiro_saldo FROM {MySql.DB_NAME}.tb_orcamentos WHERE id_empreendimento = %s AND mes_vigencia = %s AND ano_vigencia = %s ORDER BY id_orcamento"
    ...
    def consultarOrcamentoPelaData(self, idEmpreend, dtCarga):
        ...
        query = "select id_orcamento, id_empreendimento, mes_vigencia, ano_vigencia, dt_carga, item, orcado_valor, fisico_valor, fisico_percentual, fisico_saldo, financeiro_valor, financeiro_percentual, financeiro_saldo from " + \
            MySql.DB_NAME + ".tb_orcamentos where id_empreendimento = '" + \
                str(idEmpreend) + "' and dt_carga = '" + \
            dtCarga + "' order by id_orcamento"
...
#        print('+++++++++++++++++++++++++++')
    ...
    def consultarOrcamentoPeloId(self, idOrcamento):
        ...
        query = "select id_orcamento, id_empreendimento, mes_vigencia, ano_vigencia, dt_carga, item, orcado_valor, fisico_valor, fisico_percentual, fisico_saldo, financeiro_valor, financeiro_percentual, financeiro_saldo from " + \
            MySql.DB_NAME + ".tb_orcamentos where id_orcamento = '" + \
                str(idOrcamento) + "'"
...
#        print ('-----------------> ', dados)
    ...
    def carregar_orcamentos(self, caminhoArq, idEmpreend):
        ...
        while linha < l:
            ...
            query = "INSERT INTO " + MySql.DB_NAME + ".tb_orcamentos (id_empreendimento, mes_vigencia, ano_vigencia, dt_carga, item, orcado_valor, fisico_valor, fisico_percentual, fisico_saldo, financeiro_valor, financeiro_percentual, financeiro_saldo) VALUES ('" + \
                idEmpreend + "', '" + mesVigencia + "', '" + anoVigencia + "', '" + dateTime + "', '" + item + "', " + str(orcado) + ", " + str(
                    fisicoR) + ", " + str(fisicoP) + ", " + str(fisicoS) + ", " + str(financR) + ", " + str(financP) + ", " + str(financS) + ")"
...
#        print (emp)
        ...
        query = "delete from " + MySql.DB_NAME + ".tb_orcamentos" + \
            " where id_orcamento = " + str(idOrcamento)
        print(query)
...
#        print (emp)
    ...
    def salvarItemOrcamento(self, item):
        ...
        query = "update " + MySql.DB_NAME + ".tb_orcamentos set " + "item = '" + item.getItem() + "', " + "orcado_valor = " + str(item.getOrcadoValor()) + ", " + "fisico_valor = " + str(item.getFisicoValor()) + ", " + "fisico_percentual = " + str(item.getFisicoPercentual()) + ", " + "fisico_saldo = " + \
            str(item.getFisicoSaldo()) + ", " + "financeiro_valor = " + str(item.getFinanceiroValor()) + ", " + "financeiro_percentual = " + \
            str(item.getFinanceiroPercentual()) + ", " + "financeiro_saldo = " + \
            str(item.getFinanceiroSaldo()) + \
            " where id_orcamento = " + str(item.getIdOrcamento())
